...
resource "aws_ami_from_instance" "catalogue" {
  name               = "${local.common_name}-catalogue-${var.app_version}-${aws_instance.catalogue.id}" # roboshop-dev-catalogue-id
...
resource "aws_autoscaling_group" "catalogue" {
 ...
 # with in 15 mints autoscaling should be successful to launch instance s
  timeouts {
    delete = "15m"
  }
}
